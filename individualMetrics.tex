...
Rubble simply counts the number of cadences expected in each transit compared to the number that the pipeline actually used when fitting the TCE.  The pipeline gaps cadences as it iteratively searches for multiple planet systems, i.e. the transits from the first planet are removed before searching for the second transit. These gapped cadences are not counted as observations.  The Rubble metric looks at 2 transit durations and counts the number of 30-minute cadences at were observed (after gapping) and divides it by the number of cadences that were expected given the regular cadence of the Kepler instrument.  If a transit has a Rubble value greater than XX?? it is considered valid. 
