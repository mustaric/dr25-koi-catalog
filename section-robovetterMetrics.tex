...
For each TCE with five or fewer transit events contributing to the signal, $Ch_{i}$ is calculated for every transit event.  When there are more than five transit events, the individual transit events are not expected to be significantly unique down to the MES$=7.1$ threshold requirement for a TCE, and the assumptions of the Chases metric no longer apply.  The individual transit event $Ch_{i}$ values were employed in recalculating the MES (see section XX).  Transit events with $Ch_{i}<XX$ were excluded from the MES calculation.
