...
\noindent
where $d$ and $t_0$ are tunable parameters, while $\gamma$ is a positive constant. This function, known as a sigmoid (or logistic) function, has a value of 1 for $t<t_0$, 0 for $t>t_0$, and transitions quickly, but smoothly, between the two states. 

[I think the sign of the exponential term in the  denominator should be positive. Also, for a sigmoid curve, t minus t0 should be positive. When t = t0, the value of the function is d/2, and when t becomes much larger than t0, the value of the function approaches d asymptotically.] 
